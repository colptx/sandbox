function Check-IllegalCharacters ($Path, $OutputFile, [switch]$Fix, [switch]$Verbose)
{
    ...
    $invalidFileTypes = ".tmp", ".ds_store", ".one", ".pst"
    ...
    else
    {
        ...
        Add-Content $outputFile $null -Force
        ...
        foreach ($item in $items)
        {
            ...
            {
                #Add-Content -Path $outputFile -Value "$($type), $($item.Name), is $($item.Name.Length) characters (max is $($maxCharacters)) and will need to be truncated"
            ...
            {
                #Add-Content -Path $outputFile -Value "$($type), $($item.Name), is $($item.Length / 1MB) MB (max is $($maxFileSize / 1MB)) and cannot be synchronized."
            ...
            {
                #Add-Content -Path $outputFile -Value "$($type), $($item.Name), is not a valid filename for file sync."
            ...
            {
                #Add-Content -Path $outputFile -Value "$($type), $($item.Name), type $($item.Name.Substring($item.Name.Length-4)) is not a valid file type for file sync."
            ...
            if ($newFileName -ne $item.Name)
            {
                if($fix -and $valid)
                {
                    ...
                    if($Verbose)
                    {
                        Write-Host $($type) $($item.Name) hs been changed to $($newFileName) -ForegroundColor Yellow
                    }
                }
            }
        }
    }
    Write-Host "Completed Scan."
}
